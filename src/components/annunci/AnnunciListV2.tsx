...
export default function AnnunciListV2({ annunci, total, isGuest, isAdmin, filtriAttivi, page }: Props) {
  ...
  const eliminaAnnuncioAdmin = async (annId: string, titolo: string) => {
    ...
    if (res.ok) setAnnunciList(prev => prev.filter(a => a.id !== annId))
  }
    ...
    <div>
        ...
        <div style={{ textAlign: 'center', padding: '60px 20px', color: '#9ca3af' }}>
          ...
          <p style={{ fontSize: 14, margin: 0 }}>
            ...
            {!isGuest && (
              <>
                <span> </span>
                <Link href="/annunci/nuovo" style={{ color: 'var(--ms-green)', fontWeight: 600, textDecoration: 'none' }}>
      ...
      {!isGuest && totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 24, alignItems: 'center' }}>
          <button className="btn-ghost" onClick={() => goPage(page - 1)} disabled={page === 1}>
            <ChevronLeft size={16} />
          </button>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: '1px solid',
                  borderColor: p === page ? 'var(--ms-green)' : '#d0dde2',
          ...
          <button className="btn-ghost" onClick={() => goPage(page + 1)} disabled={page === totalPages}>
...
function AnnuncioCard({ ann, isGuest, onChat, delay }: { ann: AnnuncioConProfilo; isGuest: boolean; onChat: () => void; delay: number }) {
  ...
  const [mediaAperto, setMediaAperto] = React.useState<number | null>(null)
        ...
        {ann.ruoli.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8 }}>
            {ann.ruoli.slice(0, 3).map(r => (
              <span key={r} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, background: '#f0f4f5', color: '#4a6470', border: '1px solid #d8e4e8' }}>
                {r}
              </span>
            ))}
